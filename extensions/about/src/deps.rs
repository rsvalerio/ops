...
pub fn format_dependencies_section(deps: &ProjectDependencies) -> Vec<String> {
...
mod tests {
    ...
    fn format_dependencies_section_empty() {
        assert!(format_dependencies_section(&ProjectDependencies::default()).is_empty());
    }
    ...
    fn format_dependencies_section_renders_tree() {
        ...
        let out = format_dependencies_section(&deps).join("\n");
