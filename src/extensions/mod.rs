//! Optional extensions (e.g. metadata data provider).
...
pub use rust::{
    ...
    metadata,
    metadata::{Dependency, DependencyKind, Metadata, MetadataExtension, Package, Target},
    ops_db,
};

use crate::config::Config;
use crate::extension::{CommandRegistry, DataRegistry, Extension, ExtensionInfo};
...
pub fn resolve_stack(config: &Config, workspace_root: &Path) -> Option<Stack> {
    config
        ...
        .or_else(|| Stack::detect(workspace_root))
}
...
pub fn collect_compiled_extensions(
    ...
    #[allow(unused_mut)]
    let mut available: Vec<(&'static str, Box<dyn Extension>)> = Vec::new();

    #[cfg(feature = "stack-rust")]
    {
        let db_path = rust::ops_db::OpsDb::resolve_path(&config.data, workspace_root);
        available.push((
            "ops-db",
            Box::new(rust::ops_db::OpsDbExtension::new(db_path)),
        ));
        available.push(("metadata", Box::new(rust::metadata::MetadataExtension)));
        available.push((
            "cargo-toml",
            Box::new(rust::cargo_toml::CargoTomlExtension::new()),
        ));
        available.push(("about", Box::new(rust::about::AboutExtension)));
    }
...
mod tests {
    ...
    fn extension_info_provides_metadata() {
        ...
        let infos = collect_extension_info(&ext_refs);
