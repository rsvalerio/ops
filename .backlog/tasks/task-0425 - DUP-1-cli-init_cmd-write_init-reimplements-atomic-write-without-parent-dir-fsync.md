...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Promote atomic_write to pub(crate) or pub in ops_core::config and have init_cmd::write_init (force branch) delegate to it
- [x] #2 Remove the duplicated temp-name + rename logic from init_cmd.rs
- [x] #3 Non-force branch (create_new) keeps its current behaviour but its sync_all happy-path stays
<!-- AC:END -->
