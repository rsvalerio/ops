...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Either the separator is clamped to the given columns budget via truncate_to_width when columns > 0 (with a test pinning visible_width(result) <= columns for an over-long configured summary_separator), or the parameter is removed and both runner call sites updated
- [x] #2 Existing tests render_basics.rs::classic_summary_separator_is_rail and compact_summary_separator_is_empty still pass
