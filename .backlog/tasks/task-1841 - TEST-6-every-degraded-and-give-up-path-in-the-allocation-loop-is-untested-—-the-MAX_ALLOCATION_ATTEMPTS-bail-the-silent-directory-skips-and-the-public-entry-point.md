...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 a deterministic test drives commit_task_set to MAX_ALLOCATION_ATTEMPTS exhaustion and asserts the error names the tasks directory and the attempt count, and that no file from the final abandoned attempt survives
- [ ] #2 a test covers for_each_task_file against a TASK_DIRS directory that exists but cannot be read, pinning the documented 'treat as absent' policy
- [ ] #3 a test covers a non-UTF-8 filename in a scanned directory and pins that it is skipped rather than panicking or aborting the scan
- [ ] #4 run_create_review_tasks (the public entry point) is exercised by at least one test, so the wrapper and UtcStamp::now are not entirely uncovered
- [ ] #5 boundary tests exist for slugify on non-ASCII input and for next_main_task_number when the highest observed number is u32::MAX
<!-- AC:END -->
