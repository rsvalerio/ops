//! Process-local cache for the raw `pyproject.toml` text.
//!
...
static CACHE: ArcTextCache = ArcTextCache::new("pyproject.toml");

/// Read `<root>/pyproject.toml` once per process, returning the raw text
/// as a shared `Arc<str>`. Returns `None` when the file is missing.
...
mod tests {
    ...
    fn second_call_returns_same_arc() {
        let cache = ArcTextCache::new("pyproject.toml");
    ...
    fn arc_is_shared_across_two_consumer_parses() {
        ...
        // Local cache + 2 captured Arcs in test = 3 exactly (no other
        // tests share this instance).
    ...
    fn pyproject_text_returns_arc() {
        ...
        .unwrap();
        let text = pyproject_text(dir.path()).expect("text");
